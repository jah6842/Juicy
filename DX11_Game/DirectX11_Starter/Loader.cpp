...
std::shared_ptr<VertexShader> LoadVertexShader(ID3D11Device* device, VSHADER vShaderID){
	...
	case VSHADER_TEXT:
		...
		break;
	case VSHADER_2D:
		description = VERTEX_DESCRIPTION_2D;
		descriptionSize = 3;
		instanced = false;
		break;
	default:
...
std::shared_ptr<Mesh> LoadMesh(ID3D11Device* device, MESHES mesh){
	...
	if(mesh == MESH_CUBE){
		...
		m->numIndices = 6;
		m->vertexBuffer = Mesh::Create2DVertexBuffer(ButtonVertices, 4, VERTEX_TYPE_2D);
		m->numVertices = 4;
		m->vertexType = VERTEX_TYPE_2D;
		m->topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	}
	else if(mesh == MESH_PARTICLE){
		m->hasColor = true;
		m->hasNormals = false;
		m->hasPosition = true;
		m->hasTexCoord = false;
		m->indexBuffer = Mesh::CreateIndexBuffer(ButtonIndices, 6);
		m->numIndices = 6;
		m->vertexBuffer = Mesh::CreateParticleVertexBuffer(ParticleVertices, 4, VERTEX_TYPE_PARTICLE);
		m->numVertices = 4;
		m->vertexType = VERTEX_TYPE_PARTICLE;
		m->topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	}
	else 
	{
